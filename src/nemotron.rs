...
#[cfg(test)]
mod shader_guard {
    //! Registry guard for the Nemotron-specific compute kernels.
    //!
    ...
    /// Nemotron-H (Mamba2 SSD + MoE tail) kernels this model owns.
    const REQUIRED_NEMOTRON_KERNELS: &[&str] = &[
        // Mamba2 / SSD decode step
        "nemotron_ssm_conv_step",
        "nemotron_ssd_scan",
        "nemotron_gated_rmsnorm",
        // MoE routed-expert weighted accumulate
        "nemotron_moe_accum",
    ];

    #[test]
    fn nemotron_kernels_are_registered() {
        let map = crate::include_all_shaders();
        let missing: Vec<&str> = REQUIRED_NEMOTRON_KERNELS
            .iter()
            .copied()
        ...
        assert!(
            missing.is_empty(),
            "{} Nemotron shader(s) missing from the registry: {:?}\n\
    ...
    #[test]
    fn nemotron_kernel_spirv_is_wellformed() {
        let map = crate::include_all_shaders();
        for name in REQUIRED_NEMOTRON_KERNELS {
