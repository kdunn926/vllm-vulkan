...
def test_config_stub_threads_flag():
    ...
    expect(
        RustModelConfig(KIMI).trust_remote_code is False,
        "config_stub: default trust_remote_code is False",
    )
    expect(
        RustModelConfig(KIMI, trust_remote_code=True).trust_remote_code is True,
        "config_stub: trust_remote_code=True threads through",
    )


def test_resolver_branches():
    from vllm_vulkan.oai.app import resolve_trust_remote_code as resolve

    os.environ.pop("VLLM_VULKAN_TRUST_REMOTE_CODE", None)
    expect(
        resolve(KIMI, cli_flag=False) is False, "resolver: default (no cli/env) -> OFF"
    )
    expect(resolve(KIMI, cli_flag=True) is True, "resolver: CLI flag -> ON")
    for v in ("1", "true", "TRUE", "yes", "on"):
        os.environ["VLLM_VULKAN_TRUST_REMOTE_CODE"] = v
        expect(
            resolve(KIMI, cli_flag=False) is True, f"resolver: env blanket '{v}' -> ON"
        )
    for v in (KIMI, "Kimi-Linear-48B-A3B", "x/y,Kimi-Linear-48B-A3B"):
        os.environ["VLLM_VULKAN_TRUST_REMOTE_CODE"] = v
        expect(
            resolve(KIMI, cli_flag=False) is True, f"resolver: scoped hit '{v}' -> ON"
        )
    for v in ("Qwen/Qwen2.5-0.5B", "other-model"):
        os.environ["VLLM_VULKAN_TRUST_REMOTE_CODE"] = v
        expect(
            resolve(KIMI, cli_flag=False) is False,
            f"resolver: scoped miss '{v}' -> OFF",
        )
    # allow-list for a different model must not leak
    os.environ["VLLM_VULKAN_TRUST_REMOTE_CODE"] = "Qwen/Qwen2.5-0.5B"
    expect(
        resolve(KIMI, cli_flag=False) is False,
        "resolver: other-model scope does NOT enable Kimi",
    )
    os.environ.pop("VLLM_VULKAN_TRUST_REMOTE_CODE", None)
...
def test_build_app_signature():
    ...
    expect(
        ...
        "build_app: trust_remote_code default is False",
    )
...
def main():
    ...
    try:
        ...
        Exception
    ) as e:  # vllm not present (CI-lite): resolver/app tests need the venv
