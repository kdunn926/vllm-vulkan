...
async def _collect(engine, sampling_params, prompt_ids):
    outs = []
    async for ro in engine.generate(
...
def build_engine(tokenizer, ids):
    ...
    return RustEngineClient(
        vcfg, renderer=None, backend=ScriptedBackend(ids), tokenizer=tokenizer
    )
...
def mk_sp(streaming=False, **kw):
    ...
    base = {"max_tokens": 128, "temperature": 0.0, "detokenize": True}
    base.update(kw)
...
def main():
    ...
    outs = asyncio.run(
        _collect(eng, mk_sp(stop=["</s>"], include_stop_str_in_output=False), [0])
    )
    final = outs[-1].outputs[0]
    expect(
        final.text.startswith("answer here")
        and "</s>" not in final.text
        and "trailing" not in final.text
    ...
    eng = build_engine(tok, body)
    outs = asyncio.run(
        _collect(eng, mk_sp(stop=["</s>"], include_stop_str_in_output=True), [0])
    )
    final = outs[-1].outputs[0]
    expect(
        final.text.endswith("</s>") and "trailing" not in final.text,
    ...
    outs = asyncio.run(
        _collect(
            eng,
            mk_sp(streaming=True, stop=["</s>"], include_stop_str_in_output=False),
            [0],
        )
    )
    concat = "".join(o.outputs[0].text for o in outs)
    expect(
        concat.startswith("answer here")
        and "</s>" not in concat
        and "trailing" not in concat,
        f"DELTA stream concat: {concat!r}",
    )
    expect(
        outs[-1].finished and outs[-1].outputs[0].finish_reason == "stop",
    ...
    final = outs[-1].outputs[0]
    expect(
        final.finish_reason == "stop" and final.stop_reason == stop_id,
    ...
    final = outs[-1].outputs[0]
    expect(
        final.finish_reason == "length" and len(final.token_ids) == 3,
    ...
    expect(
        outs[-1].outputs[0].text == tok.decode(ids),
